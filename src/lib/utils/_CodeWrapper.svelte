<script lang="ts">
	...
	function copy() {
		const dispatched = dispatch('copy', { text: element.firstElementChild?.innerHTML });
		if (dispatched) isClipboardButtonPressed = !isClipboardButtonPressed;
	}
</script>
...
{:else}
	<div class="grid">
		...
		<div class="col-start-1 row-start-1 flex items-start justify-end p-2">
			...
			>
				<button class="btn btn-square btn-sm btn-neutral" on:click={copy}>
